...
const unsigned long pumpInterval = 2000; // Время работы насоса pH+ или pH- в миллисекундах (2 секунды)
...
const long webInterval = 30000; //таймер запуска функции выравнивания pH (30сек)
...
//вызов функции выравнивания pH каждые "webInterval" (30) секунд ТОЛЬКО при получении корректных данных с датчика рН (не nan и не inf)
  if (isPhReceived && (currentMillis - previouspHMillis >= webInterval)) {
      ...
      controlPumps(ph, currentMillis); //смотрит, входит ли значение pH в заданные рамки и запускает ph+ или ph- если ph высокий или слишком низкий
    }
